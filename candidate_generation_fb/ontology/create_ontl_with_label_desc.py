...
#{"_index":"dbontologyindex","_type":"doc","_id":"j87htGQBMToJryZHqvAb","_score":1,"_source":{"uri":"http://dbpedia.org/ontology/picture","label":"record"}}

with open("ontology_dump.json", 'a') as f_write:
    # first we are going to index entities from domain common.topic
    prev_json_item = {"_index": "fbontologyindex", "_type":"doc", "_source":{'uri':'', 'label':'', 'description':''}}

    # Preserve the triples with the ids, and get additional triples on description
    query_count = 0
    mid_uri = "http://rdf.freebase.com/ns"

    with open('fb-rdf-pred-schema-types-ids-1-byalpha-desc', 'r') as f_read_ontl:
        mid_to_look_for = ''

        while True:
            line_label_desc  = f_read_ontl.readline()
            ...
            if re.match(r'</m.[a-zA-Z0-9_]+>', mid):
                ...
                # a new mid is found
                    ...
                    prev_json_item['_source']['label'] = label_split[2]
